...
export var store = new Vuex.Store({
  ...
  mutations: {
    set_current_component_name (state, name) {
      state.current_component_name = name
    },
    ...
    set_token (state, token) {
      config.localstore.set('token', token);
      state.token = token;

      //Vue.localStorage.get('token', 'fallbackValue') // Will return 'fallbackValue' string
    }
  },
  getters: {
    has_token: (state) => (!state.token == null),
    task: (state) => (state.current_task),
    tasks: (state) => (state.tasks),
    tasks_filter: (state) => (state.tasks_filter)
  }
})
